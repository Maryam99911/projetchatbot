...
def find_similar_passages(question, contexts, threshold=0.5):
    # Obtenir les embeddings de la question
    max_similarity = -1
    question_embedding = embeddings_model.encode(question)
    ...
    for context in contexts:
        ...
        # Vérifier si la similarité est supérieure au seuil
           ...
           # Ajouter le contexte à la liste des passages similaires
        #   similar_passages.append((context, similarity))
        # Vérifier si la similarité est supérieure au maximum
    #return similar_passages
...
def extract_information(question, passages):
    ...
    #print(context)
    result  = qa_pipeline({"context": passages, "question": question})
...
def get_answer_combined(question, contexts_dict):
    ...
    if relevant_file:
        ...
        #passages = [context for context, _ in similar_passages]

        #print(passages)
        passage,similarity = find_similar_passages(question,contexts_dict[relevant_file])
        print(passage,similarity)
        # Extraire les informations pertinentes
        answers = extract_information(question, passage)
...
#question = 'qui sont les presidents de l\'association A  et de l \' association B?'
